...
struct Desc {
    ...
    explicit Desc(char const *id, quint32 size = 0) : size_{size} { set(id); }
...
struct BroadcastAudioExtension {
    ...
    BroadcastAudioExtension(Version version = Version::v_0)
        : version_{static_cast<quint16>(version)}, umid_{{}} {
...
class BWFFile::impl final {
  public:
    ...
    impl(QAudioFormat const &format)
        : header_dirty_{true}, format_{format}, header_length_{-1},
          data_size_{-1}, error_{FileError::NoError} {}

    impl(QAudioFormat const &format, QString const &name)
        : header_dirty_{true}, format_{format}, file_{name}, header_length_{-1},
          data_size_{-1}, error_{FileError::NoError} {}

    impl(QAudioFormat const &format, QString const &name,
         InfoDictionary const &dictionary)
        : header_dirty_{true}, format_{format}, file_{name},
          info_dictionary_{dictionary}, header_length_{-1}, data_size_{-1},
    ...
    BroadcastAudioExtension const *bext() const {
        return bext_.isEmpty()
                   ? nullptr
                   : reinterpret_cast<BroadcastAudioExtension const *>(
                         bext_.constData());
    }
...
bool BWFFile::impl::initialize(BWFFile *self, OpenMode mode) {
    ...
    if (mode & Append) {
        ...
        if (result)
            result = self->seek(file_.size() - header_length_);
    } else {
        result = self->seek(0);
    }
    return result;
...
bool BWFFile::impl::read_header() {
               ...
               1) // allow for uncounted pad
    {
        ...
        outer_size = be ? qFromBigEndian<quint32>(outer_desc.size_)
                        : qFromLittleEndian<quint32>(outer_desc.size_);
        if (!memcmp(&outer_desc.id_, "RIFF", 4) || be) {
            // RIFF or RIFX
            char riff_item[4];
            if (file_.read(riff_item, sizeof riff_item) != sizeof riff_item)
                return false;
            if (!memcmp(riff_item, "WAVE", 4)) {
                ...
                while (wave_offset <
                    ...
                    if (file_.read(&wave_desc, sizeof wave_desc) !=
                        ...
                        return false;
                    wave_size =
                        be ? qFromBigEndian<quint32>(wave_desc.size_)
                           : qFromLittleEndian<quint32>(wave_desc.size_);
                    ...
                    if (!memcmp(&wave_desc.id_, "fmt ", 4)) {
                        ...
                        char list_type[4];
                        if (file_.read(list_type, sizeof list_type) !=
                            sizeof list_type)
                            return false;
                        if (!memcmp(list_type, "INFO", 4)) {
                            ...
                            while (info_offset < wave_offset +
                                ...
                                if (file_.read(&info_desc, sizeof info_desc) !=
                                    sizeof info_desc)
                                    return false;
                                info_size = be ? qFromBigEndian<quint32>(
                                                     info_desc.size_)
                                               : qFromLittleEndian<quint32>(
                                                     info_desc.size_);
                                info_dictionary_[info_desc.id_] =
                                    file_.read(info_size);
                                if (!file_.seek(info_offset + sizeof info_desc +
                                                (info_size + 1) / 2 * 2))
                                    return false;
                                ;
                    ...
                    if (!file_.seek(wave_offset + sizeof wave_desc +
                                    (wave_size + 1) / 2 * 2))
                        return false;
        ...
        if (!file_.seek(outer_offset + sizeof outer_desc +
                        (outer_size + 1) / 2 * 2))
            return false;
...
bool BWFFile::impl::write_header(QAudioFormat format) {
    ...
    if (be) {
        ...
        fmt.bits_per_sample =
            qToBigEndian<quint16>(format.bytesPerSample() * 8);
        ...
        fmt.byte_rate =
            qToLittleEndian<quint32>(format.bytesForDuration(1000000));
        fmt.block_align = qToLittleEndian<quint16>(format.bytesPerFrame());
        fmt.bits_per_sample =
            qToLittleEndian<quint16>(format.bytesPerSample()) * 8;
        desc.set("fmt", qToLittleEndian<quint32>(sizeof fmt));
    }
    if (file_.write(&desc, sizeof desc) != sizeof desc)
        return false;
    if (file_.write(reinterpret_cast<char const *>(&fmt), sizeof fmt) !=
        sizeof fmt)
        return false;
...
bool BWFFile::impl::update_header() {
    if (header_length_ < 0 || !(file_.openMode() & WriteOnly))
        return false;
    ...
    if (!bext_.isEmpty()) {
        ...
        if (be) {
            data->time_reference_low_ =
                qToBigEndian<quint32>(data->time_reference_low_);
            data->time_reference_high_ =
                qToBigEndian<quint32>(data->time_reference_high_);
            switch (static_cast<BextVersion>(data->version_)) {
            case BextVersion::v_0:
                data->version_ = qToBigEndian<quint32>(data->version_);
                // fall through
            default:
                data->loudness_value_ =
                    qToBigEndian<quint16>(data->loudness_value_);
                data->loudness_range_ =
                    qToBigEndian<quint16>(data->loudness_range_);
                data->max_true_peak_level_ =
                    qToBigEndian<quint16>(data->max_true_peak_level_);
                data->max_momentary_loudness_ =
                    qToBigEndian<quint16>(data->max_momentary_loudness_);
                data->max_short_term_loudness_ =
                    qToBigEndian<quint16>(data->max_short_term_loudness_);
            }
        } else {
            data->time_reference_low_ =
                qToLittleEndian<quint32>(data->time_reference_low_);
            data->time_reference_high_ =
                qToLittleEndian<quint32>(data->time_reference_high_);
            switch (static_cast<BextVersion>(data->version_)) {
            case BextVersion::v_0:
            ...
            default:
                data->loudness_value_ =
                    qToLittleEndian<quint16>(data->loudness_value_);
                data->loudness_range_ =
                    qToLittleEndian<quint16>(data->loudness_range_);
                data->max_true_peak_level_ =
                    qToLittleEndian<quint16>(data->max_true_peak_level_);
                data->max_momentary_loudness_ =
                    qToLittleEndian<quint16>(data->max_momentary_loudness_);
                data->max_short_term_loudness_ =
                    qToLittleEndian<quint16>(data->max_short_term_loudness_);
            }
        }
        if (file_.write(bext_) != size)
            return false;
    }
    ...
    if (info_dictionary_.size()) {
        ...
        for (auto iter = info_dictionary_.constBegin();
            ...
            if (file_.write(&desc, sizeof desc) != sizeof desc)
                return false;
            if (file_.write(value.constData(), len) != len)
                return false;
        }
    ...
    if ((file_.size() % 2) && file_.seek(file_.size()) &&
        file_.write("\0", 1) != 1)
        return false;
    if (!file_.seek(0))
        return false;
...
bool BWFFile::open(OpenMode mode) {
    ...
    if (!(mode & WriteOnly)) {
        ...
        if (!(success = m_->file_.open(mode & ~Text))) {
            ...
            if (!(success =
                      (m_->read_header() || m_->write_header(m_->format_)))) {
                if (FileError::NoError == m_->file_.error()) {
...
QByteArray BWFFile::bext_description() const {
    if (!m_->bext())
        return {};
    return QByteArray::fromRawData(m_->bext()->description_,
                                   strlen(m_->bext()->description_));
}

void BWFFile::bext_description(QByteArray const &description) {
    m_->header_dirty_ = true;
    ::memcpy(m_->bext()->description_, description.constData(),
             sizeof BroadcastAudioExtension::description_);
}

QByteArray BWFFile::bext_originator() const {
    if (!m_->bext())
        return {};
    return QByteArray::fromRawData(m_->bext()->originator_,
                                   strlen(m_->bext()->originator_));
}

void BWFFile::bext_originator(QByteArray const &originator) {
    m_->header_dirty_ = true;
    ::memcpy(m_->bext()->originator_, originator.constData(),
             sizeof BroadcastAudioExtension::originator_);
}

QByteArray BWFFile::bext_originator_reference() const {
    if (!m_->bext())
        return {};
    return QByteArray::fromRawData(m_->bext()->originator_reference_,
                                   strlen(m_->bext()->originator_reference_));
}

void BWFFile::bext_originator_reference(QByteArray const &reference) {
    m_->header_dirty_ = true;
    ::memcpy(m_->bext()->originator_reference_, reference.constData(),
             sizeof BroadcastAudioExtension::originator_reference_);
}
...
QByteArray BWFFile::bext_coding_history() const {
    if (size_t(m_->bext_.size()) <= sizeof(BroadcastAudioExtension))
        return {};
    return QByteArray::fromRawData(m_->bext()->coding_history_,
                                   m_->bext_.size() -
                                       sizeof(BroadcastAudioExtension));
}
...
void BWFFile::bext_coding_history(QByteArray const &text) {
    ...
    auto end = static_cast<char const *>(
        ::memchr(text.constData(), '\0', size_t(text.size())));
...
bool BWFFile::reset() {
    ...
    if (m_->file_.isOpen()) {
        ...
        m_->bext_.clear();
        auto size = m_->data_size_ < 0 ? m_->file_.size() - m_->header_length_
                                       : m_->data_size_;
        m_->data_size_ = size;
        if (m_->header_length_ >
            qint64(3 * sizeof(Desc) + 4 + sizeof(FormatChunk))) {
...
qint64 BWFFile::size() const {
    return m_->data_size_ < 0 ? m_->file_.size() - m_->header_length_
                              : m_->data_size_;
}
...
bool BWFFile::link(QString const &link_name) {
    return m_->file_.link(link_name);
}
...
auto BWFFile::permissions() const -> Permissions {
    return m_->file_.permissions();
}
...
bool BWFFile::resize(qint64 new_size) {
    ...
    if (m_->data_size_ >= 0) {
        ...
        auto length =
            std::min(size - end_of_data, m_->file_.size() - end_of_data);
        if (length > 0) {
...
int BWFFile::handle() const {
    ...
    if (h < 0)
        const_cast<BWFFile *>(this)->setErrorString(m_->file_.errorString());
    return h;
}
